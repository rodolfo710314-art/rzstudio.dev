      ...
      <div className="w-full max-w-md border border-slate-800 bg-zinc-950">
        ...
        <div className="px-6 py-6 space-y-5">
          ...
          {result.status === "expired" && (
            <div className="font-mono text-[11px] text-amber-600/70 border border-amber-900/40 p-3 leading-relaxed">
          ...
          {result.status === "revoked" && (
            <div className="font-mono text-[11px] text-red-500/70 border border-red-900/40 p-3 leading-relaxed">
          ...
          <div className="border-t border-slate-800 pt-4">
            <p className="font-mono text-[11px] text-slate-500 lowercase leading-relaxed">
...
function Row({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex items-baseline justify-between gap-4 border-b border-slate-800/50 px-4 py-2 last:border-0">
      ...
      <span className={`font-mono text-[12px] lowercase ${highlight ? "text-amber-400" : "text-slate-300"}`}>
        {value}
      </span>
    </div>
  );
  ...
  return (
    <main className="min-h-screen bg-black flex items-center justify-center p-6">
      <div className="w-full max-w-sm border border-red-900/40 bg-zinc-950 px-6 py-8 text-center space-y-3">
        ...
        <a href="/simbiosis" className="font-mono text-[11px] text-slate-500 hover:text-[#C97352] transition-colors block">
